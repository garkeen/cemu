...
QEMU `target/i386/tcg/seg_helper.c` 的 `switch_tss_ra`（逐字段读它的 `access_ldw/ldl`
偏移），行为以 SDM vol.3 fig 7-2 裁决。16 位 TSS 与 32 位 TSS 的差别：

| | 16 位 TSS（类型 1/3） | 32 位 TSS（类型 9/B） |
|---|---|---|
| 最小 limit | **0x2b** | 0x67 |
| EIP / EFLAGS | IP@0x0e、FLAGS@0x10（各 2 字节） | EIP@0x20、EFLAGS@0x24 |
| 通用寄存器 | 0x12 起，步长 2；**只换低半，高半保留** | 0x28 起，步长 4 |
| 段选择子 | 仅 ES/CS/SS/DS@0x22 起步长 2；**FS/GS 无字段，装成 null** | ES..GS@0x48 起步长 4 |
| LDT | @0x2a | @0x60 |
| CR3 / T 位 | **都没有**（切换不动 CR3，也不会有任务切换 #DB） | CR3@0x1c、T@0x64 |

**改动**（`cpu/isa/x86/exec.c`）：`do_task_switch` 按 TSS 形状参数化字段偏移与宽度；
两处 `Fatal("16-bit TSS not implemented")` 删掉（`do_task_switch` 的类型判定收 1/3/9/0xb，
far CALL/JMP 的分派也把 1/3 送进任务切换）。`tss_stack` 本来就带 `tss32` 分支（SP0/SS0
的 16 位偏移 0x02/0x04、32 位 0x04/0x08 都对），没动。

**探针（pm t26）**：JMP 穿过一个类型 1 的 TSS，主任务先置 EAX=0xDEADBEEF、16 位 TSS 的
AX 字段置 0x1234，回来后要求 EAX==0xDEAD1234（低半来自镜像、高半保留）、任务看到的 CS
与 FLAGS 来自 0x24/0x10、且切换把任务体的 AX=0x5678 **存回 0x12**（存与取都走 16 位偏移）。
两处踩坑：

- 16 位 TSS 的 IP 只有 16 位，任务体必须落在 64K 窗口内。镜像链接在 0x10000，故新增一个
  base=0x10000/limit=0xffff 的代码描述符（SEL_CODEHI）给这个任务用。
- 写描述符时把 base 的字节位置放错（写成 0x00409b000100ffff → base 0x100），跑飞。

**测试逮住了我自己实现里的真 bug**：保存块原本用**新任务**的形状去写**旧任务**的 TSS。
从 32 位主任务切到 16 位 TSS 时，于是拿 16 位偏移写 32 位 TSS —— 0x12+2*5/2*6 正好压在
**CR3 字段（0x1c）**上，把它写成垃圾 0x10000000；返回时 CR3 装上垃圾 → 走表读到 open bus
的 0xffffffff → PDE 保留位全 1 → 触发片 19 新加的保留位检查 → #PF(ec=1) → 三重故障。
定位链全在 CEMU_DEBUG：`trap` 给 ec=1（只可能来自保留位检查）→ `mem` 给出
`ld 241c:4 -> 10000000` 与 `ld 10000000:4 -> ffffffff` → `watch=0x241c:4:w` 抓到写者就是
那条 ljmp 自己。修法：保存块用 `out16 = !(s->tr_ar & 8)`（当前 TR 的形状），装载块用
`tss16`（新任务的形状），两者分开。

**验证**：pm **26 ok / 0 bad**（rc=11）；QEMU 双跑 `pm_qemu4.txt` —— 26 格里 24 格逐字节
一致，仅存的分歧仍是 t7/t20（TCG 不执行数据段限长检查）；x86 套件 **15/0**。零告警。

**台账**：D15 整行删除。**D18（RTC 周期/闹钟中断）与 D20（i8042 命令集/扫描码集/overrun）
本轮未动**，见下条记录。
