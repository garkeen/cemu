...
static uint32_t vmask(int size) {
...
void flags_mul(uint64_t prod, int size) {
  fl->cf = fl->of = prod >> (size * 8) ? 1 : 0;
}

void flags_imul(int64_t prod, int size) {
  int64_t low = (int64_t)((uint64_t)prod << (64 - size * 8)) >>
                (64 - size * 8);
...
uint8_t fetch8(void) {
  ...
  if (fr->rec.raw_len < sizeof(fr->rec.raw))
    fr->rec.raw[fr->rec.raw_len++] = v;
  d.nxt++;
  return v;
}
...
static void modrm(void) {
  ...
  if (!d.a32) {
    ...
    static const int base[8] = {ebx_i, ebx_i, ebp_i, ebp_i,
                                esi_i, edi_i, ebp_i, ebx_i};
    static const int index[8] = {esi_i, edi_i, -1, -1, -1, -1, -1, -1};
    off = s->r[base[d.rm]].e;
    if (index[d.rm] >= 0) off += s->r[index[d.rm]].e;
    if (d.mod == 1) off += (int8_t)fetch8();
    else if (d.mod == 2) off += fetch16();
    else if (d.rm == 6) off = fetch16();  // [disp16]: no base register
    if (d.mseg < 0 && (d.rm == 2 || d.rm == 3 || (d.rm == 6 && d.mod != 0)))
      d.mseg = ss_i;
  } else if (d.rm == 4) {
...
static uint8_t reg8(void) {
  return d.reg < 4 ? s->r[d.reg].l : s->r[d.reg - 4].h;
}
static void set_reg8(uint8_t v) {
  if (d.reg < 4) s->r[d.reg].l = v;
  else s->r[d.reg - 4].h = v;
}
...
static int cond(int c) {
  int r;
  switch (c & ~1) {
    case 0x0: r = fl->of; break;
    case 0x2: r = fl->cf; break;
    case 0x4: r = fl->zf; break;
    case 0x6: r = fl->cf || fl->zf; break;
    case 0x8: r = fl->sf; break;
    case 0xa: r = fl->pf; break;
    case 0xc: r = fl->sf != fl->of; break;
    default:  r = fl->zf || fl->sf != fl->of; break;  // 0xe
  }
  return r ^ (c & 1);
}
...
static uint32_t sar(uint32_t v, uint32_t cnt, int size) {
  ...
  uint32_t r = cnt >= width ? fill
                            : ((v >> cnt) | (fill << (width - cnt))) & vmask(size);
...
static uint32_t grp1_math(int kind, uint32_t a, uint32_t b, int size) {
  ...
  uint32_t r = 0;
  switch (kind) {
    case 0: r = (a + b) & vmask(size); flags_add(a, b, r, size, 0); break;
    case 2: r = (a + b + carry) & vmask(size); flags_add(a, b, r, size, carry); break;
    case 5: case 7: r = (a - b) & vmask(size); flags_sub(a, b, r, size, 0); break;
    case 3: r = (a - b - carry) & vmask(size); flags_sub(a, b, r, size, carry); break;
    case 1: r = a | b; flags_logic(r, size); break;
    case 4: r = a & b; flags_logic(r, size); break;
    default: r = a ^ b; flags_logic(r, size); break;  // 6: xor
  }
  return r;
}
...
static void grp1(int size, int imm_form) {
  ...
  modrm();
  uint32_t src;
  if (imm_form == 1) src = (uint32_t)(int8_t)imm8();
  else if (imm_form == 2) src = imm8();
  else src = d.w32 ? imm32() : imm16();
  if (size == 1) {
...
static void grp2(int size, uint32_t cnt) {
  ...
  if (size == 1) {
    v = RM8();
    switch (d.reg) {
      case 0: r = rol(v, cnt, 1); break;
      case 1: r = ror(v, cnt, 1); break;
      case 2: r = rcl(v, cnt, 1); break;
      case 3: r = rcr(v, cnt, 1); break;
      case 4: case 6: r = shl(v, cnt, 1); break;
      case 5: r = shr(v, cnt, 1); break;
      default: r = sar(v, cnt, 1); break;
    }
    if (cnt) SET_RM8(r);  // count 0 changes nothing, not even the store path
  } else if (size == 2) {
    v = RM16();
    switch (d.reg) {
      case 0: r = rol(v, cnt, 2); break;
      case 1: r = ror(v, cnt, 2); break;
      case 2: r = rcl(v, cnt, 2); break;
      case 3: r = rcr(v, cnt, 2); break;
      case 4: case 6: r = shl(v, cnt, 2); break;
      case 5: r = shr(v, cnt, 2); break;
      default: r = sar(v, cnt, 2); break;
    }
    if (cnt) SET_RM16(r);
  } else {
    v = RM32();
    switch (d.reg) {
      case 0: r = rol(v, cnt, 4); break;
      case 1: r = ror(v, cnt, 4); break;
      case 2: r = rcl(v, cnt, 4); break;
      case 3: r = rcr(v, cnt, 4); break;
      case 4: case 6: r = shl(v, cnt, 4); break;
      case 5: r = shr(v, cnt, 4); break;
      default: r = sar(v, cnt, 4); break;
    }
    if (cnt) SET_RM32(r);
  }
}
...
static void idiv_w(uint32_t src) {
  ...
  uint32_t lo = d.w32 ? eax : ax;
  int64_t dividend =
      (int64_t)((((uint64_t)(d.w32 ? edx : dx) << (d.w32 ? 32 : 16)) | lo) &
  ...
  if (q < -(int64_t)(1LL << (bits - 1)) || q >= (int64_t)(1LL << (bits - 1)))
    de();
...
static void grp3(int size) {
  ...
  if (d.reg == 2) {  // not: no flags
    if (size == 1) SET_RM8(~RM8());
    else if (size == 2) SET_RM16(~RM16());
    else SET_RM32(~RM32());
    return;
  }
  if (d.reg == 3) {  // neg
  ...
  if (size == 1) {
    switch (d.reg) {
      ...
      case 5: imul8(); return;
      case 6: div8(); return;
      default: idiv8(); return;
    }
  }
  uint32_t src = size == 2 ? RM16() : RM32();
  switch (d.reg) {
    case 4: mul_w(src); return;
    case 5: imul_w(src); return;
    case 6: div_w(src); return;
    default: idiv_w(src); return;
  }
}
...
static void string_op(uint8_t op) {
  ...
  switch (op) {
    case 0xa4: case 0xa5:  // movs
      if (size == 1) wr8(dlin, rd8(slin));
      else if (size == 2) wr16(dlin, rd16(slin));
      else wr32(dlin, rd32(slin));
      break;
    case 0xa6: case 0xa7: {  // cmps: [si] - [di]
    ...
    case 0xaa: case 0xab:  // stos
      if (size == 1) wr8(dlin, al);
      else if (size == 2) wr16(dlin, ax);
      else wr32(dlin, eax);
      break;
    case 0xac: case 0xad:  // lods
      if (size == 1) al = rd8(slin);
      else if (size == 2) ax = rd16(slin);
      else eax = rd32(slin);
      break;
  ...
  if (str_uses_si(op)) {
    if (d.a32) esi += (uint32_t)step;
    else si = (uint16_t)(sio + step);
  }
  if (str_uses_di(op)) {
    if (d.a32) edi += (uint32_t)step;
    else di = (uint16_t)(dio + step);
  }
  if (!d.rep) return;
  uint32_t cnt = (d.w32 ? ecx : cx) - 1;
  if (d.w32) ecx = cnt;
  else cx = (uint16_t)cnt;
  if (cnt == 0) return;
...
static void run_op2(uint8_t op2) {
  switch (op2) {
    case 0x01: {  // grp6: sgdt/sidt/lgdt/lidt/smsw (reg field)
      modrm();
      switch (d.reg) {
        case 0: case 1: {  // sgdt/sidt: store limit(2) then base(4)
        ...
        case 2: case 3: {  // lgdt/lidt
          ...
          if (d.reg == 2) { s->gdtr = base; s->gdtr_limit = limit; }
          else { s->idtr = base; s->idtr_limit = limit; }
          break;
        }
        case 4:  // smsw: CR0 at the rm width
          if (d.is_mem) {
            if (d.w32) wr32(d.mlin, s->cr0);
            else wr16(d.mlin, (uint16_t)s->cr0);
          } else {
            if (d.w32) set_rm32(s->cr0);
            else set_rm16((uint16_t)s->cr0);
          }
          break;
        default:
    ...
    case 0x0b: ud(); break;  // ud2
    case 0x1f: modrm(); break;  // multi-byte nop
    ...
    case 0xa0: push_w(s->sreg[fs_i]); break;          // push fs
    case 0xa1: load_seg(fs_i, (uint16_t)pop_w()); break;  // pop fs
    case 0xa2: {  // cpuid
      uint32_t leaf = eax;
      if (leaf == 0) {
        eax = 1; ebx = 0x756e6547; edx = 0x49656e69; ecx = 0x6c65746e;
      } else if (leaf == 1) {
        // Family 6, model 3, stepping 3; no FPU, no TSC.
        eax = 0x633; ebx = ecx = edx = 0;
      } else {
        eax = ebx = ecx = edx = 0;
      }
      break;
    }
    case 0xa3: case 0xab: case 0xb3: case 0xbb: {  // bt/bts/btr/btc rm, reg
      modrm();
      uint32_t v, bit, pos;
      if (d.w32) {
        v = RM32();
      ...
      fl->cf = (v >> pos) & 1;
      if (op2 == 0xab) v |= 1u << pos;
      else if (op2 == 0xb3) v &= ~(1u << pos);
      else if (op2 == 0xbb) v ^= 1u << pos;
      if (op2 != 0xa3) {
        if (d.w32) SET_RM32(v);
        else SET_RM16(v);
      }
    ...
    case 0xa4: case 0xa5: {  // shld rm, reg, imm8/cl
      modrm();
    ...
    case 0xac: case 0xad: {  // shrd rm, reg, imm8/cl
      modrm();
    ...
    case 0xb2: case 0xb4: case 0xb5: {  // lss/lfs/lgs r, m16:16/32
      modrm();
      if (!d.is_mem) ud();
    ...
    case 0xba: {  // grp8: bt/bts/btr/btc rm, imm8 (reg field 4-7)
      ...
      fl->cf = (v >> pos) & 1;
      if (d.reg == 5) v |= 1u << pos;
      else if (d.reg == 6) v &= ~(1u << pos);
      else if (d.reg == 7) v ^= 1u << pos;
      if (d.w32) SET_RM32(v);
      else SET_RM16(v);
      break;
    }
    case 0xbe: {  // movsx r16/32, rm8
      ...
      if (d.w32) set_reg32(v);
      else set_reg16((uint16_t)v);
      break;
    }
    case 0xbf:  // movsx r32, rm16
      ...
      break;
    case 0xc0: case 0xc1: {  // xadd rm, reg
      modrm();
      if (op2 & 1) {
        if (d.w32) {
          uint32_t dst = RM32(), src = reg32();
    ...
    case 0xcc: case 0xcd: case 0xce: case 0xcf: {  // bswap r32
      ...
      s->r[op2 & 7].e = ((v & 0xff) << 24) | ((v & 0xff00) << 8) |
                        ((v >> 8) & 0xff00) | (v >> 24);
      break;
    }
    default:
...
void run_op(uint8_t op) {
  if (op == 0x0f) { run_op2(fetch8()); return; }  // two-byte escape (SDM 0f)
  switch (op) {
    case 0x00: { modrm(); uint8_t a = RM8(), b = reg8(); uint8_t r = a + b; SET_RM8(r); flags_add(a, b, r, 1, 0); break; }  // add rm8, r8
    case 0x01: { modrm(); if (d.w32) { uint32_t a = RM32(), b = reg32(); uint32_t r = a + b; SET_RM32(r); flags_add(a, b, r, 4, 0); } else { uint16_t a = RM16(), b = reg16(); uint16_t r = a + b; SET_RM16(r); flags_add(a, b, r, 2, 0); } break; }  // add rm, r
    case 0x02: { modrm(); uint8_t a = reg8(), b = RM8(); uint8_t r = a + b; set_reg8(r); flags_add(a, b, r, 1, 0); break; }  // add r8, rm8
    case 0x03: { modrm(); if (d.w32) { uint32_t a = reg32(), b = RM32(); uint32_t r = a + b; set_reg32(r); flags_add(a, b, r, 4, 0); } else { uint16_t a = reg16(), b = RM16(); uint16_t r = a + b; set_reg16(r); flags_add(a, b, r, 2, 0); } break; }  // add r, rm
    case 0x04: { uint8_t a = al, b = imm8(); al = a + b; flags_add(a, b, al, 1, 0); break; }  // add al, imm8
    case 0x05: { if (d.w32) { uint32_t a = eax, b = imm32(); eax = a + b; flags_add(a, b, eax, 4, 0); } else { uint16_t a = ax, b = imm16(); ax = a + b; flags_add(a, b, ax, 2, 0); } break; }  // add eAX, imm
    case 0x06: push_w(s->sreg[es_i]); break;  // push es
    case 0x07: load_seg(es_i, (uint16_t)pop_w()); break;  // pop es
    case 0x08: { modrm(); uint8_t a = RM8(), b = reg8(); uint8_t r = a | b; SET_RM8(r); flags_logic(r, 1); break; }  // or rm8, r8
    case 0x09: { modrm(); if (d.w32) { uint32_t a = RM32(), b = reg32(); uint32_t r = a | b; SET_RM32(r); flags_logic(r, 4); } else { uint16_t a = RM16(), b = reg16(); uint16_t r = a | b; SET_RM16(r); flags_logic(r, 2); } break; }  // or rm, r
    case 0x0a: { modrm(); uint8_t a = reg8(), b = RM8(); uint8_t r = a | b; set_reg8(r); flags_logic(r, 1); break; }  // or r8, rm8
    case 0x0b: { modrm(); if (d.w32) { uint32_t a = reg32(), b = RM32(); uint32_t r = a | b; set_reg32(r); flags_logic(r, 4); } else { uint16_t a = reg16(), b = RM16(); uint16_t r = a | b; set_reg16(r); flags_logic(r, 2); } break; }  // or r, rm
    case 0x0c: { uint8_t a = al, b = imm8(); al = a | b; flags_logic(al, 1); break; }  // or al, imm8
    case 0x0d: { if (d.w32) { uint32_t a = eax, b = imm32(); eax = a | b; flags_logic(eax, 4); } else { uint16_t a = ax, b = imm16(); ax = a | b; flags_logic(ax, 2); } break; }  // or eAX, imm
    case 0x0e: push_w(s->sreg[cs_i]); break;  // push cs
    case 0x10: { modrm(); uint8_t a = RM8(), b = reg8(); int cin = fl->cf; uint8_t r = (uint8_t)(a + b + cin); SET_RM8(r); flags_add(a, b, r, 1, cin); break; }  // adc rm8, r8
    case 0x11: { modrm(); int cin = fl->cf; if (d.w32) { uint32_t a = RM32(), b = reg32(); uint32_t r = a + b + cin; SET_RM32(r); flags_add(a, b, r, 4, cin); } else { uint16_t a = RM16(), b = reg16(); uint16_t r = (uint16_t)(a + b + cin); SET_RM16(r); flags_add(a, b, r, 2, cin); } break; }  // adc rm, r
    case 0x12: { modrm(); uint8_t a = reg8(), b = RM8(); int cin = fl->cf; uint8_t r = (uint8_t)(a + b + cin); set_reg8(r); flags_add(a, b, r, 1, cin); break; }  // adc r8, rm8
    case 0x13: { modrm(); int cin = fl->cf; if (d.w32) { uint32_t a = reg32(), b = RM32(); uint32_t r = a + b + cin; set_reg32(r); flags_add(a, b, r, 4, cin); } else { uint16_t a = reg16(), b = RM16(); uint16_t r = (uint16_t)(a + b + cin); set_reg16(r); flags_add(a, b, r, 2, cin); } break; }  // adc r, rm
    case 0x14: { uint8_t a = al, b = imm8(); int cin = fl->cf; al = (uint8_t)(a + b + cin); flags_add(a, b, al, 1, cin); break; }  // adc al, imm8
    case 0x15: { int cin = fl->cf; if (d.w32) { uint32_t a = eax, b = imm32(); eax = a + b + cin; flags_add(a, b, eax, 4, cin); } else { uint16_t a = ax, b = imm16(); ax = (uint16_t)(a + b + cin); flags_add(a, b, ax, 2, cin); } break; }  // adc eAX, imm
    case 0x16: push_w(s->sreg[ss_i]); break;  // push ss
    case 0x17: load_seg(ss_i, (uint16_t)pop_w()); break;  // pop ss
    case 0x18: { modrm(); uint8_t a = RM8(), b = reg8(); int cin = fl->cf; uint8_t r = (uint8_t)(a - b - cin); SET_RM8(r); flags_sub(a, b, r, 1, cin); break; }  // sbb rm8, r8
    case 0x19: { modrm(); int cin = fl->cf; if (d.w32) { uint32_t a = RM32(), b = reg32(); uint32_t r = a - b - cin; SET_RM32(r); flags_sub(a, b, r, 4, cin); } else { uint16_t a = RM16(), b = reg16(); uint16_t r = (uint16_t)(a - b - cin); SET_RM16(r); flags_sub(a, b, r, 2, cin); } break; }  // sbb rm, r
    case 0x1a: { modrm(); uint8_t a = reg8(), b = RM8(); int cin = fl->cf; uint8_t r = (uint8_t)(a - b - cin); set_reg8(r); flags_sub(a, b, r, 1, cin); break; }  // sbb r8, rm8
    case 0x1b: { modrm(); int cin = fl->cf; if (d.w32) { uint32_t a = reg32(), b = RM32(); uint32_t r = a - b - cin; set_reg32(r); flags_sub(a, b, r, 4, cin); } else { uint16_t a = reg16(), b = RM16(); uint16_t r = (uint16_t)(a - b - cin); set_reg16(r); flags_sub(a, b, r, 2, cin); } break; }  // sbb r, rm
    case 0x1c: { uint8_t a = al, b = imm8(); int cin = fl->cf; al = (uint8_t)(a - b - cin); flags_sub(a, b, al, 1, cin); break; }  // sbb al, imm8
    case 0x1d: { int cin = fl->cf; if (d.w32) { uint32_t a = eax, b = imm32(); eax = a - b - cin; flags_sub(a, b, eax, 4, cin); } else { uint16_t a = ax, b = imm16(); ax = (uint16_t)(a - b - cin); flags_sub(a, b, ax, 2, cin); } break; }  // sbb eAX, imm
    case 0x1e: push_w(s->sreg[ds_i]); break;  // push ds
    case 0x1f: load_seg(ds_i, (uint16_t)pop_w()); break;  // pop ds
    case 0x20: { modrm(); uint8_t a = RM8(), b = reg8(); uint8_t r = a & b; SET_RM8(r); flags_logic(r, 1); break; }  // and rm8, r8
    case 0x21: { modrm(); if (d.w32) { uint32_t a = RM32(), b = reg32(); uint32_t r = a & b; SET_RM32(r); flags_logic(r, 4); } else { uint16_t a = RM16(), b = reg16(); uint16_t r = a & b; SET_RM16(r); flags_logic(r, 2); } break; }  // and rm, r
    case 0x22: { modrm(); uint8_t a = reg8(), b = RM8(); uint8_t r = a & b; set_reg8(r); flags_logic(r, 1); break; }  // and r8, rm8
    case 0x23: { modrm(); if (d.w32) { uint32_t a = reg32(), b = RM32(); uint32_t r = a & b; set_reg32(r); flags_logic(r, 4); } else { uint16_t a = reg16(), b = RM16(); uint16_t r = a & b; set_reg16(r); flags_logic(r, 2); } break; }  // and r, rm
    case 0x24: { uint8_t a = al, b = imm8(); al = a & b; flags_logic(al, 1); break; }  // and al, imm8
    case 0x25: { if (d.w32) { uint32_t a = eax, b = imm32(); eax = a & b; flags_logic(eax, 4); } else { uint16_t a = ax, b = imm16(); ax = a & b; flags_logic(ax, 2); } break; }  // and eAX, imm
    case 0x27: daa(); break;  // daa
    case 0x28: { modrm(); uint8_t a = RM8(), b = reg8(); uint8_t r = a - b; SET_RM8(r); flags_sub(a, b, r, 1, 0); break; }  // sub rm8, r8
    case 0x29: { modrm(); if (d.w32) { uint32_t a = RM32(), b = reg32(); uint32_t r = a - b; SET_RM32(r); flags_sub(a, b, r, 4, 0); } else { uint16_t a = RM16(), b = reg16(); uint16_t r = (uint16_t)(a - b); SET_RM16(r); flags_sub(a, b, r, 2, 0); } break; }  // sub rm, r
    case 0x2a: { modrm(); uint8_t a = reg8(), b = RM8(); uint8_t r = a - b; set_reg8(r); flags_sub(a, b, r, 1, 0); break; }  // sub r8, rm8
    case 0x2b: { modrm(); if (d.w32) { uint32_t a = reg32(), b = RM32(); uint32_t r = a - b; set_reg32(r); flags_sub(a, b, r, 4, 0); } else { uint16_t a = reg16(), b = RM16(); uint16_t r = (uint16_t)(a - b); set_reg16(r); flags_sub(a, b, r, 2, 0); } break; }  // sub r, rm
    case 0x2c: { uint8_t a = al, b = imm8(); al = (uint8_t)(a - b); flags_sub(a, b, al, 1, 0); break; }  // sub al, imm8
    case 0x2d: { if (d.w32) { uint32_t a = eax, b = imm32(); eax = a - b; flags_sub(a, b, eax, 4, 0); } else { uint16_t a = ax, b = imm16(); ax = (uint16_t)(a - b); flags_sub(a, b, ax, 2, 0); } break; }  // sub eAX, imm
    case 0x2f: das(); break;  // das
    case 0x30: { modrm(); uint8_t a = RM8(), b = reg8(); uint8_t r = a ^ b; SET_RM8(r); flags_logic(r, 1); break; }  // xor rm8, r8
    case 0x31: { modrm(); if (d.w32) { uint32_t a = RM32(), b = reg32(); uint32_t r = a ^ b; SET_RM32(r); flags_logic(r, 4); } else { uint16_t a = RM16(), b = reg16(); uint16_t r = a ^ b; SET_RM16(r); flags_logic(r, 2); } break; }  // xor rm, r
    case 0x32: { modrm(); uint8_t a = reg8(), b = RM8(); uint8_t r = a ^ b; set_reg8(r); flags_logic(r, 1); break; }  // xor r8, rm8
    case 0x33: { modrm(); if (d.w32) { uint32_t a = reg32(), b = RM32(); uint32_t r = a ^ b; set_reg32(r); flags_logic(r, 4); } else { uint16_t a = reg16(), b = RM16(); uint16_t r = a ^ b; set_reg16(r); flags_logic(r, 2); } break; }  // xor r, rm
    case 0x34: { uint8_t a = al, b = imm8(); al = a ^ b; flags_logic(al, 1); break; }  // xor al, imm8
    case 0x35: { if (d.w32) { uint32_t a = eax, b = imm32(); eax = a ^ b; flags_logic(eax, 4); } else { uint16_t a = ax, b = imm16(); ax = a ^ b; flags_logic(ax, 2); } break; }  // xor eAX, imm
    case 0x37: aaa(); break;  // aaa
    case 0x38: { modrm(); uint8_t a = RM8(), b = reg8(); flags_sub(a, b, (uint8_t)(a - b), 1, 0); break; }  // cmp rm8, r8
    case 0x39: { modrm(); if (d.w32) { uint32_t a = RM32(), b = reg32(); flags_sub(a, b, a - b, 4, 0); } else { uint16_t a = RM16(), b = reg16(); flags_sub(a, b, (uint16_t)(a - b), 2, 0); } break; }  // cmp rm, r
    case 0x3a: { modrm(); uint8_t a = reg8(), b = RM8(); flags_sub(a, b, (uint8_t)(a - b), 1, 0); break; }  // cmp r8, rm8
    case 0x3b: { modrm(); if (d.w32) { uint32_t a = reg32(), b = RM32(); flags_sub(a, b, a - b, 4, 0); } else { uint16_t a = reg16(), b = RM16(); flags_sub(a, b, (uint16_t)(a - b), 2, 0); } break; }  // cmp r, rm
    case 0x3c: { uint8_t a = al, b = imm8(); flags_sub(a, b, (uint8_t)(a - b), 1, 0); break; }  // cmp al, imm8
    case 0x3d: { if (d.w32) { uint32_t a = eax, b = imm32(); flags_sub(a, b, a - b, 4, 0); } else { uint16_t a = ax, b = imm16(); flags_sub(a, b, (uint16_t)(a - b), 2, 0); } break; }  // cmp eAX, imm
    case 0x3f: aas(); break;  // aas
    case 0x40: case 0x41: case 0x42: case 0x43:
    case 0x44: case 0x45: case 0x46: case 0x47: {  // inc r
      if (d.w32) { uint32_t r = s->r[op & 7].e + 1; s->r[op & 7].e = r; flags_inc(r, 4); }
      else { uint16_t r = (uint16_t)(s->r[op & 7].x + 1); s->r[op & 7].x = r; flags_inc(r, 2); }
      break;
    }
    case 0x48: case 0x49: case 0x4a: case 0x4b:
    case 0x4c: case 0x4d: case 0x4e: case 0x4f: {  // dec r
      if (d.w32) { uint32_t r = s->r[op & 7].e - 1; s->r[op & 7].e = r; flags_dec(r, 4); }
      else { uint16_t r = (uint16_t)(s->r[op & 7].x - 1); s->r[op & 7].x = r; flags_dec(r, 2); }
      break;
    }
    ...
    case 0x5c: case 0x5d: case 0x5e: case 0x5f:  // pop r
      if (d.w32) s->r[op & 7].e = pop32();
      else s->r[op & 7].x = pop16();
      break;
    case 0x60: {  // pusha: ax cx dx bx sp bp si di, sp = the ORIGINAL value
      ...
      if (d.w32) {
        ...
        push32(sp0); push32(ebp); push32(esi); push32(edi);
      } else {
        push16(ax); push16(cx); push16(dx); push16(bx);
        push16((uint16_t)sp0); push16(bp); push16(si); push16(di);
      }
      break;
    }
    case 0x61: {  // popa: di si bp (skip sp) bx dx cx ax
      if (d.w32) {
        edi = pop32(); esi = pop32(); ebp = pop32(); (void)pop32();
        ebx = pop32(); edx = pop32(); ecx = pop32(); eax = pop32();
      } else {
        di = pop16(); si = pop16(); bp = pop16(); (void)pop16();
        bx = pop16(); dx = pop16(); cx = pop16(); ax = pop16();
      }
      break;
    }
    case 0x68: push_w(d.w32 ? imm32() : imm16()); break;  // push imm
    case 0x69: {  // imul r, rm, imm
      modrm();
      if (d.w32) {
        int64_t prod = (int64_t)(int32_t)(d.is_mem ? rd32(d.mlin) : s->r[d.rm].e) * (int32_t)imm32();
        set_reg32((uint32_t)prod);
        flags_imul(prod, 4);
      } else {
        int32_t prod = (int32_t)(int16_t)(d.is_mem ? rd16(d.mlin) : s->r[d.rm].x) * (int16_t)imm16();
        set_reg16((uint16_t)prod);
        flags_imul(prod, 2);
      }
      break;
    }
    case 0x6a: push_w((uint32_t)(int32_t)(int8_t)imm8()); break;  // push imm8 (sign-extended)
    ...
    case 0x80: grp1(1, 2); break;
    case 0x81: grp1(d.w32 ? 4 : 2, 0); break;
    case 0x82: grp1(1, 2); break;
    case 0x83: grp1(d.w32 ? 4 : 2, 1); break;
    case 0x84: { modrm(); uint8_t a = RM8(), b = reg8(); flags_logic(a & b, 1); break; }  // test rm8, r8
    case 0x85: { modrm(); if (d.w32) { uint32_t a = RM32(), b = reg32(); flags_logic(a & b, 4); } else { uint16_t a = RM16(), b = reg16(); flags_logic(a & b, 2); } break; }  // test rm, r
    case 0x86: { modrm(); uint8_t t = RM8(); SET_RM8(reg8()); set_reg8(t); break; }  // xchg rm8, r8
    case 0x87: { modrm(); if (d.w32) { uint32_t t = RM32(); SET_RM32(reg32()); set_reg32(t); } else { uint16_t t = RM16(); SET_RM16(reg16()); set_reg16(t); } break; }  // xchg rm, r
    case 0x88: { modrm(); SET_RM8(reg8()); break; }  // mov rm8, r8
    case 0x89: { modrm(); if (d.w32) SET_RM32(reg32()); else SET_RM16(reg16()); break; }  // mov rm, r
    case 0x8a: { modrm(); set_reg8(RM8()); break; }  // mov r8, rm8
    case 0x8b: { modrm(); if (d.w32) set_reg32(RM32()); else set_reg16(RM16()); break; }  // mov r, rm
    case 0x8c: { modrm(); if (d.w32) SET_RM32(s->sreg[d.reg]); else SET_RM16(s->sreg[d.reg]); break; }  // mov rm16, sreg
    case 0x8d: {  // lea r, m
      modrm();
      if (!d.is_mem) ud();
      if (d.w32) set_reg32(d.moff);
      else set_reg16((uint16_t)d.moff);
      break;
    }
    case 0x8e: { modrm(); if (d.reg > gs_i) ud(); load_seg(d.reg, rm16()); break; }  // mov sreg, rm16
    case 0x8f: { modrm(); uint32_t v = pop_w(); if (d.w32) SET_RM32(v); else SET_RM16((uint16_t)v); break; }  // pop rm
    case 0x90: break;  // nop
    case 0x91: case 0x92: case 0x93: case 0x94:
    case 0x95: case 0x96: case 0x97: {  // xchg eAX, r
      if (d.w32) { uint32_t t = eax; eax = s->r[op & 7].e; s->r[op & 7].e = t; }
      else { uint16_t t = ax; ax = s->r[op & 7].x; s->r[op & 7].x = t; }
      break;
    }
    case 0x98:  // cbw/cwde: sign-extend AL->AX or AX->EAX
      if (d.w32) eax = (uint32_t)(int32_t)(int16_t)ax;
      else ax = (uint16_t)(int16_t)(int8_t)al;
      break;
    case 0x99:  // cwd/cdq: sign-extend AX->DX:AX or EAX->EDX:EAX
      if (d.w32) edx = (int32_t)eax < 0 ? 0xffffffffu : 0;
      else dx = (int16_t)ax < 0 ? 0xffffu : 0;
      break;
    ...
    case 0x9b: break;  // wait: no x87 in this machine
    case 0x9c: push_w(fl->word | 2); break;  // pushf
    case 0x9d: {  // popf: bit 1 stays set; RF/VM never load (SDM)
      uint32_t v = pop_w();
      if (d.w32) fl->word = (v & ~(0x30000u)) | 2;
      else fl->word = (fl->word & 0xffff0000u) | (v | 2);
      break;
    }
    ...
    case 0x9f: ah = (uint8_t)(fl->word & 0xd5u) | 2; break;  // lahf
    case 0xa0: { uint64_t lin = s->base[d.seg >= 0 ? d.seg : ds_i] + (d.a32 ? imm32() : imm16()); al = rd8(lin); break; }  // mov al, moffs8
    case 0xa1: { uint64_t lin = s->base[d.seg >= 0 ? d.seg : ds_i] + (d.a32 ? imm32() : imm16()); if (d.w32) eax = rd32(lin); else ax = rd16(lin); break; }  // mov eAX, moffs
    case 0xa2: { uint64_t lin = s->base[d.seg >= 0 ? d.seg : ds_i] + (d.a32 ? imm32() : imm16()); wr8(lin, al); break; }  // mov moffs8, al
    case 0xa3: { uint64_t lin = s->base[d.seg >= 0 ? d.seg : ds_i] + (d.a32 ? imm32() : imm16()); if (d.w32) wr32(lin, eax); else wr16(lin, ax); break; }  // mov moffs, eAX
    case 0xa4: case 0xa5: case 0xa6: case 0xa7:
    case 0xaa: case 0xab: case 0xac: case 0xad:
    case 0xae: case 0xaf:
      string_op(op);
      break;
    case 0xa8: { uint8_t a = al, b = imm8(); flags_logic(a & b, 1); break; }  // test al, imm8
    case 0xa9: { if (d.w32) { uint32_t a = eax, b = imm32(); flags_logic(a & b, 4); } else { uint16_t a = ax, b = imm16(); flags_logic(a & b, 2); } break; }  // test eAX, imm
    case 0xb0: case 0xb1: case 0xb2: case 0xb3:
    case 0xb4: case 0xb5: case 0xb6: case 0xb7:  // mov r8, imm8
      if (op < 0xb4) s->r[op & 7].l = imm8();
      else s->r[op & 3].h = imm8();
      break;
    case 0xb8: case 0xb9: case 0xba: case 0xbb:
    case 0xbc: case 0xbd: case 0xbe: case 0xbf:  // mov r, imm
      if (d.w32) s->r[op & 7].e = imm32();
      else s->r[op & 7].x = imm16();
      break;
    case 0xc0: grp2(1, imm8()); break;
    case 0xc1: grp2(d.w32 ? 4 : 2, imm8()); break;
    case 0xc2: {  // ret imm16
      uint32_t n = imm16();
      eip = pop_w();
      if (d.w32) esp += n; else sp = (uint16_t)(sp + n);
      break;
    }
    case 0xc3: eip = pop_w(); break;  // ret
    case 0xc4: case 0xc5: {  // les/lds r, m16:16/32
      modrm();
      if (!d.is_mem) ud();
      int seg = op == 0xc4 ? es_i : ds_i;
      if (d.w32) { set_reg32(RM32()); load_seg(seg, rd16(d.mlin + 4)); }
      else { set_reg16(RM16()); load_seg(seg, rd16(d.mlin + 2)); }
      break;
    }
    case 0xc6: { modrm(); SET_RM8(imm8()); break; }  // mov rm8, imm8
    case 0xc7: { modrm(); if (d.w32) SET_RM32(imm32()); else SET_RM16(imm16()); break; }  // mov rm, imm
    case 0xc8: {  // enter imm16, imm8 (SDM ENTER)
      ...
      if (level > 0) {
        for (uint32_t i = 1; i < level; i++) {
          if (d.w32) { ebp -= 4; esp = ebp; push32(rd32(s->base[ss_i] + esp + 4 - 4)); }
          else { bp -= 2; sp = bp; push16(rd16(s->base[ss_i] + sp + 2 - 2)); }
        }
        push_w(frame);
      }
      if (d.w32) { ebp = frame; esp -= alloc; }
      else { bp = (uint16_t)frame; sp = (uint16_t)(sp - alloc); }
      break;
    }
    case 0xc9: {  // leave: sp = bp, then pop bp
      if (d.w32) { esp = ebp; ebp = pop32(); }
      else { sp = bp; bp = pop16(); }
      break;
    }
    ...
    case 0xcb: eip = pop_w(); load_seg(cs_i, (uint16_t)pop_w()); break;  // retf
    case 0xcc: do_int(3, (uint32_t)(fr->rec.pc + d.nxt)); return;  // int3
    case 0xcd: do_int(imm8(), (uint32_t)(fr->rec.pc + d.nxt)); return;  // int imm8
    case 0xce: if (fl->of) { do_int(4, (uint32_t)(fr->rec.pc + d.nxt)); return; } break;  // into
    case 0xcf: {  // iret
      eip = pop_w();
      load_seg(cs_i, (uint16_t)pop_w());
      uint32_t flv = pop_w();
    ...
    case 0xd0: grp2(1, 1); break;
    case 0xd1: grp2(d.w32 ? 4 : 2, 1); break;
    case 0xd2: grp2(1, cl); break;
    case 0xd3: grp2(d.w32 ? 4 : 2, cl); break;
    ...
    case 0xd6: al = fl->cf ? 0xff : 0x00; break;  // salc (undocumented)
    case 0xd7: {  // xlat: AL = [seg:BX + AL]
      uint64_t lin = s->base[d.seg >= 0 ? d.seg : ds_i] +
                     (d.a32 ? ebx : (uint32_t)bx) + al;
      al = rd8(lin);
      break;
    }
    ...
    case 0xe0: case 0xe1: case 0xe2: {  // loopne/loope/loop rel8
      int8_t rel = (int8_t)imm8();
      uint32_t cnt = (d.w32 ? ecx : cx) - 1;
      if (d.w32) ecx = cnt; else cx = (uint16_t)cnt;
    ...
    case 0xe4: { uint16_t port = imm8(); al = (uint8_t)BusRead(cpu->io, port, 1); break; }  // in al, imm8
    case 0xe5: { uint16_t port = imm8(); if (d.w32) eax = BusRead(cpu->io, port, 4); else ax = (uint16_t)BusRead(cpu->io, port, 2); break; }  // in eAX, imm8
    case 0xe6: BusWrite(cpu->io, imm8(), 1, al); break;  // out imm8, al
    case 0xe7: BusWrite(cpu->io, imm8(), d.w32 ? 4 : 2, d.w32 ? eax : ax); break;  // out imm8, eAX
    ...
    case 0xeb: { int8_t rel = (int8_t)imm8(); d.nxt += (uint32_t)(int32_t)rel; break; }  // jmp rel8
    case 0xec: al = (uint8_t)BusRead(cpu->io, dx, 1); break;  // in al, dx
    case 0xed: if (d.w32) eax = BusRead(cpu->io, dx, 4); else ax = (uint16_t)BusRead(cpu->io, dx, 2); break;  // in eAX, dx
    case 0xee: BusWrite(cpu->io, dx, 1, al); break;  // out dx, al
    case 0xef: BusWrite(cpu->io, dx, d.w32 ? 4 : 2, d.w32 ? eax : ax); break;  // out dx, eAX
    ...
    case 0xf5: fl->cf = !fl->cf; break;  // cmc
    case 0xf8: fl->cf = 0; break;  // clc
    case 0xf9: fl->cf = 1; break;  // stc
    case 0xfa: fl->if_ = 0; break;  // cli
    case 0xfb:  // sti: the next instruction is not interruptible (SDM)
      fl->if_ = 1;
      s->intr_inhibit = 1;
      break;
    case 0xfc: fl->df = 0; break;  // cld
    case 0xfd: fl->df = 1; break;  // std
    case 0xf6: grp3(1); break;
    case 0xf7: grp3(d.w32 ? 4 : 2); break;
    case 0xfe: {  // grp4: inc/dec rm8 (reg field)
      ...
      SET_RM8(r);
      if (d.reg) flags_dec(r, 1); else flags_inc(r, 1);
      break;
    }
      ...
      switch (d.reg) {
        case 0: case 1: {
          if (d.w32) {
            ...
            SET_RM32(r);
            if (d.reg) flags_dec(r, 4); else flags_inc(r, 4);
          } else {
            uint16_t v = RM16();
            uint16_t r = (uint16_t)(d.reg ? v - 1 : v + 1);
            SET_RM16(r);
            if (d.reg) flags_dec(r, 2); else flags_inc(r, 2);
          }
          break;
        }
        case 2: {  // call near rm
        ...
        case 4: eip = d.w32 ? RM32() : RM16(); break;  // jmp near rm
        ...
        case 6: push_w(d.w32 ? RM32() : RM16()); break;  // push rm
        default: ud();
      }
      break;
    }
...
void x86_init(CpuState *c) {
  ...
  for (uint64_t a = c->image_base; a < c->image_base + 0x2000; a += 4)
    if (BusRead(c->bus, a, 4) == k_mb_hdr_magic) { multiboot = 1; break; }
  if (multiboot) {
    st->cr0 = 1;  // PE
    st->sreg[cs_i] = 0x08;
    st->base[cs_i] = 0;
...
void x86_dump_regs(const CpuState *c) {
  ...
  for (int i = 0; i < 8; i += 4) {
    int n = snprintf(buf, sizeof(buf), "%s=%08x %s=%08x %s=%08x %s=%08x\n",
                     names[i], st->r[i].e, names[i + 1], st->r[i + 1].e,
                     names[i + 2], st->r[i + 2].e, names[i + 3],
                     st->r[i + 3].e);
  ...
  int n = snprintf(buf, sizeof(buf),
                   ...
                   "%s=%04x %s=%04x %s=%04x\n",
                   (unsigned)c->pc, st->fl.word, st->cr0, snames[0],
                   st->sreg[0], snames[1], st->sreg[1], snames[2],
                   st->sreg[2], snames[3], st->sreg[3], snames[4],
