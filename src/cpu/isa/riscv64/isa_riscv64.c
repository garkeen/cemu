...
void riscv_dump_regs(const CpuState* cpu) {
  ...
  for (int i = 0; i < 32; i += 4) {
    int n =
        snprintf(buf, sizeof(buf), "%4s=%016llx %4s=%016llx %4s=%016llx %4s=%016llx\n", names[i],
                 (unsigned long long)cpu->gpr[i], names[i + 1], (unsigned long long)cpu->gpr[i + 1],
                 names[i + 2], (unsigned long long)cpu->gpr[i + 2], names[i + 3],
                 (unsigned long long)cpu->gpr[i + 3]);
    HostWriteErr(buf, (size_t)n);
  }
  int n = snprintf(buf, sizeof(buf), "  pc=%016llx mstatus=%016llx mcause=%016llx mtval=%016llx\n",
                   (unsigned long long)cpu->pc, (unsigned long long)s->mstatus,
                   (unsigned long long)s->mcause, (unsigned long long)s->mtval);
  HostWriteErr(buf, (size_t)n);
}
...
static const char* riscv_cause_name(uint64_t cause) {
  ...
  switch (cause) {
    case kExFetchMisaligned:
      return "fetch-misalign";
    case kExFetchFault:
      return "fetch-fault";
    case kExIllegal:
      return "illegal";
    case kExBreakpoint:
      return "breakpoint";
    case kExLoadMisaligned:
      return "load-misalign";
    case kExLoadFault:
      return "load-fault";
    case kExStoreMisaligned:
    ...
    case kExMachineEcall:
      return "m-ecall";
    case kExFetchPageFault:
      return "fetch-page";
    case kExLoadPageFault:
      return "load-page";
    case kExStorePageFault:
      return "store-page";
    default:
      return NULL;
  }
...
static void riscv_debug_state_line(char* buf, int cap, frame* f) {
  RiscvState* s = f->priv;
  snprintf(buf, (size_t)cap, "x: priv=%d mstatus=%016llx mie=%016llx mip=%016llx wait=%d", s->priv,
           (unsigned long long)s->mstatus, (unsigned long long)s->mie, (unsigned long long)s->mip,
           f->cpu->wait);
}
