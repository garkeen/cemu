...
void riscv_deliver_interrupt(CpuState *cpu, RiscvState *s) {
  ...
  if (cpu->timer_read && s->stimecmp != ~0ULL)
    riscv_set_ext_irq(cpu, kIrqStip,
...
int riscv_csr_read(const CpuState *cpu, RiscvState *s, uint64_t addr,
  ...
  switch (addr) {
    ...
    case 0x744: *out = s->mnstatus; return 0;  // mnstatus (Smrnig)
...
int riscv_csr_write(RiscvState *s, uint64_t addr, uint64_t val,
                  uint64_t next_pc) {
  if (CsrPriv(addr) > s->priv) return -1;
  ...
  switch (addr) {
    ...
    case 0x7A1:  // tdata1: type and dmode hold, the rest is WARL-stored
      s->tdata[0] = (s->tdata[0] & (kTrigTypeMask | (1ULL << 59))) |
                    (val & kTrigWmask);
      return 0;
    case 0x744: s->mnstatus = val; return 0;    // mnstatus (Smrnig, WARL)
