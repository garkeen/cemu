...
static void OnClintIrq(void *ctx, int line, int level) {
  ...
  RiscvSetExtIrq(cpu, line == kClintLineMsip ? kIrqMsip : kIrqMtip, level);
}
...
static void OnPlicIrq(void *ctx, int line, int level) {
  ...
  RiscvSetExtIrq(cpu, line == 0 ? kIrqMeip : kIrqSeip, level);
}
...
static void VirtPoll(Machine *m) {
  VirtMachine *vm = (VirtMachine *)m;
  ClintPoll(&vm->clint);
}

// MachineDestroy frees the Machine itself; this hook releases the extra
// allocations only.
static void VirtDestroy(Machine *m) {
  VirtMachine *vm = (VirtMachine *)m;
  RamDestroy(vm->mrom);
}
...
Machine *VirtMachineCreate(const MachineOpts *opts) {
  ...
  VirtMachine *vm = (VirtMachine *)calloc(1, sizeof(VirtMachine));
  if (!vm) return NULL;
  Machine *m = &vm->base;
  m->name = "virt";
