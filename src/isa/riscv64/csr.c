...
// from M mode (priv spec 3.5, xiangshanNEMU intr.c intr_deleg_S).
void RiscvTrap(CpuState *cpu, RiscvState *s, uint64_t cause, uint64_t tval) {
  if (getenv("CEMU_TRACE")) {
    char buf[128];
    int n = snprintf(buf, sizeof(buf),
                     "[trap] pc=%016llx cause=%llu tval=%016llx\n",
  ...
  if (to_s) {
    ...
    st &= ~kMstatusSie;
    st = (st & ~kMstatusSpp) |
         (s->priv == kPrivSupervisor ? kMstatusSpp : 0);
    s->mstatus = st;
    ...
    st &= ~kMstatusMie;
    st = (st & ~kMstatusMppMask) |
         ((uint64_t)s->priv << 11);  // MPP <- interrupted privilege
    s->mstatus = st;
    s->priv = kPrivMachine;
...
void RiscvDeliverPendingInterrupt(CpuState *cpu, RiscvState *s) {
  ...
  if (cpu->timer_read && s->stimecmp != ~0ULL)
    RiscvSetExtIrq(cpu, kIrqStip,
                   cpu->timer_read(cpu->timer_dev) >= s->stimecmp);
  ...
  for (size_t i = 0; i < sizeof(kOrder) / sizeof(kOrder[0]); i++) {
    ...
    if (delegated) {
      ...
      take = (s->priv == kPrivSupervisor)
                 ? ((s->mstatus & kMstatusSie) != 0)
                 : (s->priv < kPrivSupervisor);
    } else {
      // Targets M: gated by MIE only when trapping from M itself.
      take = (s->priv == kPrivMachine)
                 ? ((s->mstatus & kMstatusMie) != 0)
                 : (s->priv < kPrivMachine);
    }
    if (take) {
      RiscvTrap(cpu, s, kIntBit | (uint64_t)id, 0);
      return;
    }
  }
}
...
static int ReadCounter(const CpuState *cpu, RiscvState *s, uint64_t addr,
                       uint64_t *out) {
  if (s->priv < kPrivMachine) {
    uint64_t gate = 0;
    switch (addr) {
      case 0xC00: gate = kCounterenCycle; break;
      case 0xC01: gate = kCounterenTime; break;
      case 0xC02: gate = kCounterenInstret; break;
      default: break;
    }
...
int RiscvCsrRead(const CpuState *cpu, RiscvState *s, uint64_t addr,
  ...
  switch (addr) {
    ...
    case 0x104: *out = s->mie & kIrqSLevelMask; return 0;  // sie view
    case 0x105:
      *out = s->stvec;
      return 0;
    case 0x106: *out = s->scounteren; return 0;
    case 0x10A: *out = s->senvcfg; return 0;
    case 0x14D: *out = s->stimecmp; return 0;  // Sstc (RV64, no *h CSR)
    ...
    case 0x144: *out = MipAll(s) & kIrqSLevelMask; return 0;  // sip view
    case 0x180:
    ...
    case 0x30A: *out = s->menvcfg; return 0;
    ...
    case 0x344: *out = MipAll(s); return 0;
    case 0x7A0: *out = s->tselect; return 0;       // tselect
    case 0x7A1: *out = s->tdata[0]; return 0;      // tdata1
    case 0x7A2: *out = s->tdata[1]; return 0;      // tdata2
    case 0x7A3: *out = s->tdata[2]; return 0;      // tdata3
    case 0x7A4: *out = 1ULL << 6; return 0;        // tinfo: mcontrol6 only
...
static void WritePmpAddr(RiscvState *s, int i, uint64_t val) {
  ...
  uint8_t next_cfg =
      (i + 1 < kPmpCount)
          ? (uint8_t)(s->pmpcfg[(i + 1) / 8] >> (8 * ((i + 1) % 8)))
          : 0;
  ...
  if ((cfg & kPmpL) || ((next_cfg & kPmpL) && (next_cfg & kPmpAMask) == kPmpATor))
    return;
  s->pmpaddr[i] = val & ((1ULL << kPmpAddrBits) - 1);
}

int RiscvCsrWrite(RiscvState *s, uint64_t addr, uint64_t val,
                  uint64_t next_pc) {
  if (CsrPriv(addr) > s->priv) return -1;
  ...
  switch (addr) {
    ...
    case 0x105:
      ...
      s->stvec = val & ~2ULL;
      return 0;
    case 0x106: s->scounteren = val; return 0;
    case 0x10A: s->senvcfg = val & kSenvcfgWmask; return 0;
    case 0x14D: s->stimecmp = val; return 0;  // Sstc: retires a pending STIP
    ...
    case 0x180: {  // satp
      ...
      if (s->priv == kPrivSupervisor && (s->mstatus & kMstatusTvm))
        return -1;
    ...
    case 0x302: s->medeleg = val & kMedelegWmask; return 0;
    case 0x303: s->mideleg = val & kMidelegWmask; return 0;
    case 0x304: s->mie = val; return 0;
    case 0x305:
      s->mtvec = val & ~2ULL;
      return 0;
    case 0x306: s->mcounteren = val; return 0;
    case 0x30A: s->menvcfg = val & kMenvcfgWmask; return 0;
    ...
    case 0x7A1:  // tdata1: type and dmode hold, the rest is WARL-stored
      s->tdata[0] = (s->tdata[0] & (kTrigTypeMask | (1ULL << 59))) |
                    (val & kTrigWmask);
      return 0;
    case 0x7A2: s->tdata[1] = val; return 0;      // tdata2
    case 0x7A3: s->tdata[2] = val; return 0;      // tdata3
