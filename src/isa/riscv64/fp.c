...
void RiscvTracePush(RiscvState *s, uint64_t pc, uint32_t inst, int len) {
  s->trace[s->trace_pos].pc = pc;
  s->trace[s->trace_pos].inst = inst;
  s->trace[s->trace_pos].len = (uint8_t)len;
  s->trace_pos = (s->trace_pos + 1) % 64;
  if (s->trace_count < 64) s->trace_count++;
}

void RiscvTraceDump(const RiscvState *s) {
  char buf[128];
  HostWriteErr("--- last instructions ---\n", 26);
  int start = (s->trace_pos - s->trace_count + 64) % 64;
  for (int i = 0; i < s->trace_count; i++) {
    int idx = (start + i) % 64;
    int n = snprintf(buf, sizeof(buf), "pc=%016llx inst=%08x\n",
                     (unsigned long long)s->trace[idx].pc,
                     s->trace[idx].inst);
