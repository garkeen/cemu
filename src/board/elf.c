...
static void FindHtifSymbols(const uint8_t* data, size_t size, uint64_t shoff, uint16_t shentsize,
                            uint16_t shnum, uint32_t shstrndx, ElfInfo* out) {
  if (shoff == 0 || shnum == 0 || shoff + (uint64_t)shnum * shentsize > size) return;
...
int ElfLoad(Bus* bus, const uint8_t* data, size_t size, ElfInfo* out) {
  ...
  for (uint16_t i = 0; i < phnum; i++) {
    ...
    if (!out->image_base || p_vaddr < out->image_base) out->image_base = p_vaddr;
    uint8_t* host = NULL;
    if (BusRamRange(bus, p_vaddr, p_memsz, &host) != 0) {
      LogError("elf: segment [%llx, %llx) outside RAM", (unsigned long long)p_vaddr,
  ...
  if (is64) FindHtifSymbols(data, size, shoff, shentsize, shnum, shstrndx, out);
  return 0;
}
