...
void BusAddRegion(Bus *bus, uint64_t base, uint64_t size,
                  const DeviceOps *ops, void *dev) {
...
void BusAddRamRegion(Bus *bus, uint64_t base, uint64_t size,
                     const DeviceOps *ops, void *dev, uint8_t *host) {
...
uint64_t BusRead(Bus *bus, uint64_t addr, int size) {
  ...
  if (!r)
    Fatal("read of unmapped address %llx", (unsigned long long)addr);
...
void BusWrite(Bus *bus, uint64_t addr, int size, uint64_t val) {
  BusRegion *r = FindRegion(bus, addr, size);
  if (!r)
    Fatal("write of unmapped address %llx", (unsigned long long)addr);
