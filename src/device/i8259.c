...
static void PicUpdateIrq(PicDevice *pic) {
  ...
  if (pic->set_irq && irq >= 0)
    pic->set_irq(pic->irq_ctx, irq, 1);
}
...
static void PicIoWrite(PicDevice *pic, PicState *s, uint16_t addr,
                       uint8_t val) {
  if (addr == kPortCmd) {
    if (val & 0x10) {
      // ICW1: begin init.
      ...
      s->single_mode = val & 2;
      if (val & 0x08)
        LogError("i8259: level-sensitive mode not supported");
    } else if (val & 0x08) {
      // OCW3.
...
static uint64_t PicMasterRead(void *dev, uint64_t addr, int size) {
  (void)size;
  return PicIoRead((PicDevice *)dev, &((PicDevice *)dev)->pics[0],
                   (uint16_t)(addr & 1));
}
static void PicMasterWrite(void *dev, uint64_t addr, int size,
                           uint64_t val) {
  (void)size;
  PicIoWrite((PicDevice *)dev, &((PicDevice *)dev)->pics[0],
             (uint16_t)(addr & 1), (uint8_t)val);
}
static uint64_t PicSlaveRead(void *dev, uint64_t addr, int size) {
  (void)size;
  return PicIoRead((PicDevice *)dev, &((PicDevice *)dev)->pics[1],
                   (uint16_t)(addr & 1));
}
static void PicSlaveWrite(void *dev, uint64_t addr, int size,
                          uint64_t val) {
  (void)size;
  PicIoWrite((PicDevice *)dev, &((PicDevice *)dev)->pics[1],
             (uint16_t)(addr & 1), (uint8_t)val);
}

static const DeviceOps kPicMasterOps = {"8259-master", PicMasterRead,
                                        PicMasterWrite};
static const DeviceOps kPicSlaveOps = {"8259-slave", PicSlaveRead,
                                       PicSlaveWrite};
...
void PicRegister(Bus *io, PicDevice *pic, uint16_t master_base,
                 uint16_t slave_base) {
...
void PicSetIrqSink(PicDevice *pic, void (*set_irq)(void *, int, int),
                   void *ctx) {
  pic->set_irq = set_irq;
  pic->irq_ctx = ctx;
}
