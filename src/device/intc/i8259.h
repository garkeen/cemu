...
// Semantics follow the QEMU i8259 model (Fabrice Bellard, MIT): IRR, fixed
// priority by rotating `priority_add`, auto-EOI and special fully nested mode,
// ICW1..4 init sequence. IRQs 0..7 feed the master, 8..15 the slave (whose INT
// line drives master IRQ2).
//
// A line is edge-triggered unless it is switched to level, which ICW1's LTIM
// bit does for a whole chip and the PIIX3's edge/level control register
// (PicRegisterElcr) does line by line.
//
...
// The edge/level control register the PIIX3 exposes at 0x4D0/0x4D1.
...
// (0..15; 8..15 land on the slave whose INT feeds master IRQ2).
