...
static uint64_t ClintRead(void* dev, uint64_t addr, int size) {
  ...
  if (addr >= c->base + kClintOffMsip && addr < c->base + kClintOffMsip + 4) return c->msip & 1;
  if (addr >= c->base + kClintOffMtimecmp && addr < c->base + kClintOffMtimecmp + 8)
    return SubRead64(c->mtimecmp, addr - (c->base + kClintOffMtimecmp), size);
  if (addr >= c->base + kClintOffMtime && addr < c->base + kClintOffMtime + 8)
...
static void ClintWrite(void* dev, uint64_t addr, int size, uint64_t val) {
  ClintDevice* c = (ClintDevice*)dev;
  if (addr >= c->base + kClintOffMsip && addr < c->base + kClintOffMsip + 4) {
    c->msip = (uint32_t)(val & 1);
    UpdateMsip(c);
    return;
  }
  if (addr >= c->base + kClintOffMtimecmp && addr < c->base + kClintOffMtimecmp + 8) {
    c->mtimecmp = SubWrite64(c->mtimecmp, addr - (c->base + kClintOffMtimecmp), size, val);
