...
static int IsTohost(HtifDevice *h, uint64_t addr) {
  return addr - h->tohost_addr < 8;
}
...
static void HtifProcess(HtifDevice *h, uint64_t val) {
  ...
  if (dev == 0 && cmd == 0) {
    if (payload & 1) {
      ...
      LogError("htif: syscall payload %llx not supported",
  ...
  if (dev == 1 && cmd == 1) {
    ...
    LogError("htif: unhandled dev=%u cmd=%u payload=%llx", dev, cmd,
             (unsigned long long)payload);
    h->cpu->halted = kCpuExited;
    h->cpu->exit_code = 1;
  }
...
static void HtifWrite(void *dev, uint64_t addr, int size, uint64_t val) {
  ...
  if (v) {
    ...
    if (!h->cpu->halted)
      h->fromhost = ((v >> 56) << 56) | (((v >> 48) & 0xff) << 48);
  }
}
