...
CEMU_DEBUG = item[,item...]
  ...
  mark                      无帧事件行：板级/设备级/解释器级观测，`a` 十进制、
                            ...
                            aux-byte aux-irq(IRQ12) hostmouse mouse-inj key-inj
                            reset-req(设备请求机器复位)
  mouse=DX:DY:BUTTONS[:WHEEL]@N
  key=SCAN[:EXT[:UP]]@N
                            合成宿主输入（探针作为客人按不了鼠标、也按不了键，
                            宿主事件路径只有这一条自动化通道；QEMU 侧的对应物
                            是监视器的 mouse_move/mouse_button/sendkey）。每 N
                            条指令往板级的 sink 送一次：mouse= 给的是位移/滚轮
                            （有符号 C 字面量，十六进制要写 0x）与按键位（bit0
                            左、bit1 右、bit2 中）；key= 给的是宿主报的 set-1
                            扫描码 + EXT（0xe0 前缀）+ UP（释放），也就是窗口
                            键盘交给 sink 的那个三元组。@N 必填：缺省的节奏
                            等于替调用方做决定
...
| D17 | cga.c 渲染与光栅时序缺口 | **没有 VGA 家族核**：只有独立的一份 CGA（16KB 单平面、0x3D0-0x3DF、0xB8000），没有 VGA 家族核（4 平面 / 64K+ 显存 / 0x3C0-0x3DF 四组寄存器 CRTC-Sequencer-Graphics-Attribute / 平面与链式两种寻址 / 256 色 DAC），因此 EGA 640×350（验收 4 要的）与 VGA 各模式都点不亮；CGA 图形模式（0x3D8 bit1）不渲染（黑屏）；过扫描边框不渲染（视频禁止时填黑）；0x3DA 回扫状态为宿主时钟近似（262 行×63.6µs 帧模型，行内只分活跃/消隐两相，非逐像素光栅）；属性/光标闪烁取固定场倍数周期，不跟随场相位；显示通道（display.h 的 DisplaySourceOps 宽高是设备生命周期常量、host/display_win.c 的窗口不可缩放）无法随模式换尺寸 | IBM CGA Technical Reference；IBM EGA Technical Reference；FreeVGA；QEMU hw/display/vga.c 行为旁证 | 阶段 4 图形切片：**VGA 家族核 + CGA/EGA 兼容模式**（一个核，CGA/EGA 是它的兼容模式层，验收 4 要的 EGA 640×350 在其中；见 arch.md 阶段 4）；需要精确光栅时序的软件（raster 技巧 demo）出现时再校准 | 登记中 |
...
| D21 | 软盘与 DMA 通路缺失 | 无 Intel 8272 软盘控制器、无 8237 DMA（通道 2 给软盘、通道 0 给内存刷新），CMOS 设备字节也不报软驱；再加上 8042 软驱数据线语义，整条"软盘引导 + PC 兼容传软盘"的路径都不存在。后果：Linux 0.11/0.12 那类把引导码写死成 DL=0（且要求每道 15/18 扇区）的软盘引导镜像无法引导 —— 只能走硬盘/光盘引导的镜像 | PC/AT Technical Reference（FDC 命令集）；Intel 8237A datasheet；QEMU hw/block/fdc.c + hw/dma/i8257.c；SeaBIOS src/hw/floppy.c（INT 13h 路径） | 验收 4（Windows 1.01 的 1.44MB 软盘镜像，QEMU 基线为 MS-DOS Executive 桌面）的硬前置，阶段 4 下一片；oldlinux 的 0.11/0.12 套件同样受它阻塞 | 登记中 |
| D32 | uart16550.c 接收侧 FIFO 语义 | `PushRx` 无视 FCR.FE 一直缓存 16 字节：实机 16550 在 FIFO 关闭时只保留 1 字节（RBR），后续字节按溢出丢弃并置 LSR.OE，本机不丢。后果：不开 FIFO 的客户机驱动在本机也能收全突发（sermouse 探针原先正是靠这处偏差通过的），在实机/QEMU 上会丢字节 | 16550D datasheet（FCR.FE、RBR 单字节与 OE）；QEMU hw/char/serial.c `serial_receive1` | 需要精确接收溢出语义的客户机出现时（sermouse 探针已改为开 FIFO，销账无阻塞） | 登记中 |
