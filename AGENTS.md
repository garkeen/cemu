...
| D31 | `test/x86/kut_access.c`（kvm-unit-tests `x86/access.c` 的 32 位移植） | **枚举范围与轴被收窄**：上游是 64 位专属（`x86/Makefile.i386` 把 `access.flat` 注释成 "doesn't compile"），移植去掉本机不存在的轴——NX（PTE/PDE bit63 + EFER.NXE）、PTE/PDE bit51、PKU（AD/WD/PKEY）、SMEP；去掉 KVM-MMU 专属的 `AC_ACCESS_TWICE` 与三个 `ac_test_cases`（corrupt_hugepage_triger / check_pfec_on_prefetch_pte / check_smep_andnot_wp）；accessed/dirty 轴固定置位（A/D 的**更新**规则由 `test/x86/pm` t17 覆盖）。flag 空间从上游 27 位（2^27）收到 11 位（2048 组合）——cemu 解释执行约 1.4 MIPS（200M 条指令 2m26s），全空间在本机不可跑 | oracle 与判据是上游原码：`ac_test_permissions` 的 `kwritable = !CR0.WP && !user` 即 SDM vol.3 §4.6 表 4-2 的直译；64 位专属性的出处是 `x86/Makefile.i386` 的注释 | 需要更全的轴（A/D 与 PSE 的交叉、SMEP/PKU/NX）时：等真 64 位模式，或在 pm 套件里按轴补 | 登记中 |
