...
| D20 | i8042/PS/2 键盘 | 键盘**设备**的命令集只记录不回答（无 0xFA 应答、set-scancode-set 不切换、0xFF 自检不应答），所以 SeaBIOS 的 PS/2 探测仍超时（此前即有，无害）；扫描码一律按 set 1 发（命令字节的翻译位只存不译）；无鼠标（AUX）；输出队列满时丢字节（无 overrun 位） | PC/AT Technical Reference；队列深度与三条中断条件对 QEMU pckbd.c（KBD_QUEUE_SIZE / kbd_update_irq） | 需要 set 2 键盘、鼠标或真正走 PS/2 设备命令的客户机时 | 登记中 |
